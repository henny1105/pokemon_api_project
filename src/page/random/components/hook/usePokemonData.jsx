		...
		const fetchData = async () => {
			try {
				...
				for (let i = 0; i < responses.length; i += 2) {
					...
					const evolutionChainResponse = await axios.get(speciesInfo.evolution_chain.url); // 진화 정보 가져오기
					const isBaseForm = evolutionChainResponse.data.chain.species.name === speciesInfo.name; // 기본 폼 여부 확인
